...
7. In the Quick Launch Wizard:
	...
	4. Make sure the server selected is **Ubuntu Server 12.04.1 LTS** (which is *free tier eligible*). Click **continue**
...
The first time you connect to your instance, TextWrangler may warn you that it can't authenticate the server (part of computer security is not just you proving your identity to a server, but the server proving its identity for you). For now, this is OK, allow TextWrangler to connect.
...
(but use your instance hostname, which will be similar but with different numers). You may see something like:
...
Type `yes` (in the "real world" we'd be a bit more paranoid, but for now...).
...
# copy from the EC2 instance to S3
...
The upload to S3 segment does something a bit fancier in the `for page` loop than I presented in class.
...
Character sets can get tricky. ASCII is the simplest. UTF-8 is the new standard for the web, and includes lots of languages and symbols. If you are using any non-ASCII characters (anything but letters, numbers, and basic punctuation), it is best in your web page to specify this. Put the following in the `head` of the page:
...
To link to an image, use a HTML tag such as

{% highlight html %}
<img src="picture.jpg"/>
{% endhighlight %}

Then prepare your picture (save it as a JPEG), and upload it to the same S3 bucket as your HTML file. You can also link to images on other websites, by figuring out their web address in your browser and using this as the `src`:

{% highlight html %}
<img src="http://www1.ccny.cuny.edu/images/logo2.jpg"/>
{% endhighlight %}

## Assignment for next class

Make fan fiction pages dedicated to your favorite website. Make more than one page. `View Source` is your friend.
